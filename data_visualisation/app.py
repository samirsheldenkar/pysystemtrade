    ...
    try:
        ...
        if file_path.endswith('.parquet'):
            try:
                ...
                st.error("Error reading parquet. Ensure pyarrow or fastparquet is installed.")
                return None
...
def main():
    ...
    ref_folder = st.sidebar.selectbox("Reference Folder", available_folders, index=ref_idx, key='ref_folder')
    comp_folder = st.sidebar.selectbox("Comparison Folder", available_folders, index=comp_idx, key='comp_folder')
    ...
    if df_comp is not None:
        ...
        if df_ref is not None and not df_ref.empty and not df_comp.empty:
            ...
            if common_cols:
                ...
                try:
                    # Align on index
                    df_ref_aligned, df_comp_aligned = df_ref.align(df_comp, join='inner')
                    
                    if not df_ref_aligned.empty:
                        diff = df_ref_aligned[common_cols] - df_comp_aligned[common_cols]
                        
                        st.subheader("Differences (Ref - Comp)")
                        st.dataframe(diff.describe())
                        
                        diff_col = st.selectbox("Select Column for Difference Plot", common_cols, key='diff_col')
                        fig_diff = px.line(diff, y=diff_col, title=f"Difference: {diff_col}")
        ...
        st.warning(f"File {selected_file} not found or could not be loaded in {comp_folder}")
